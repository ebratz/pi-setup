...
function roomDir(): string {
	return path.join(COMS_DIR, "rooms");
}

function roomFilePath(roomId: string): string {
	return path.join(roomDir(), `${roomId}.json`);
}

function readRoom(roomId: string): RoomInfo | null {
	try {
		const raw = fs.readFileSync(roomFilePath(roomId), "utf-8");
...
function writeRoom(room: RoomInfo): void {
	fs.mkdirSync(roomDir(), { recursive: true });
	const fp = roomFilePath(room.id);
	const tmp = `${fp}.tmp`;
	fs.writeFileSync(tmp, JSON.stringify(room, null, 2));
	fs.renameSync(tmp, fp);
}

function findRoomByName(name: string): RoomInfo | null {
	const dir = roomDir();
	...
	for (const f of files) {
		...
		try {
			...
			const parsed = JSON.parse(raw) as RoomInfo;
			if (parsed && parsed.id && parsed.name === name) return parsed;
		} catch { /* skip */ }
	}
	return null;
}

function ensureDefaultRoom(agentName: string): RoomInfo {
	// 1. Try to find an existing room named "default" by scanning all room files.
	let room = findRoomByName(DEFAULT_ROOM_NAME);
	if (room) return room;
	// 2. Fall back to the well-known hardcoded ID (legacy / first-boot).
	room = readRoom(DEFAULT_ROOM_ID);
	if (room) return room;
	// 3. Create a brand-new default room with the well-known ID.
	room = {
		id: DEFAULT_ROOM_ID,
...
function joinOrCreateRoom(roomName: string, agentName: string): RoomInfo {
	let room = findRoomByName(roomName);
	if (!room) {
		room = {
			id: ulid(),
			name: roomName,
...
export default function (pi: ExtensionAPI) {
	...
	// NOTE: var (not let) — jiti 2.7 has a closure bug with let in command handlers.
	// When a command handler references a let scoped to the extension factory function,
	// jiti may drop the binding, causing "ReferenceError: <var> is not defined".
	// var has function-level hoisting which jiti handles correctly.
	var identity: {
		session_id: string;
		name: string;
		purpose: string;
	...
	var currentRoomId: string = DEFAULT_ROOM_ID;

	// Must be defined AFTER identity declaration so the closure captures
	// the variable correctly (jiti hoisting + let TDZ workaround).
	function updateOwnRoomId(roomId: string): void {
		if (!identity) return;
		try {
			const entries = readAllRegistryEntries(identity.project);
			const own = entries.find(e => e.session_id === identity!.session_id);
			if (own) {
				own.room_id = roomId;
				writeRegistryAtomic(own, identity!.project);
				identity!.registryFile = registryFilePath(identity!.project, own.name);
			}
		} catch { /* best-effort */ }
	}
	...
	var pingTimer: NodeJS.Timeout | null = null;
	var keepaliveTimer: NodeJS.Timeout | null = null;
	var includeExplicit = false;
	var displayProject: string | null = null;
	var currentCtx: ExtensionContext | null = null;
	var currentInbound: InboundContext | null = null;
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		// 3b. Auto-join default room — always look up the "default" room by
		//     name in the filesystem so its real ID is used.
		const defaultRoom = ensureDefaultRoom(name);
		...
		try {
			const roomName = readRoom(currentRoomId)?.name ?? DEFAULT_ROOM_NAME;
			ctx.ui.setStatus("coms", `📡 ${name}@${project} 🏠 ${roomName}`);
			installPoolWidget(ctx);
			ctx.ui.notify(
				`📡 coms ready · ${name}@${project} · room "${roomName}"`,
				"info",
			);
		} catch {
	...
	function renderPool(width: number, theme: Theme): string[] {
		...
		// Build a session_id → room_id map from registry for room-filtering live peers
		const sessionRoomMap = new Map<string, string>();
		for (const entry of registryEntries) {
			sessionRoomMap.set(entry.session_id, entry.room_id ?? DEFAULT_ROOM_ID);
		}
		...
		for (const [sid, card] of peerCards.entries()) {
			...
			// Room isolation for live peers — use registry room_id when available
			const peerRoom = sessionRoomMap.get(sid) ?? DEFAULT_ROOM_ID;
			if (identity && peerRoom !== identity.roomId) continue;
		...
		for (const entry of registryEntries) {
			...
			// Room isolation: only show peers in the same room.
			if (identity && entry.room_id !== identity.roomId) continue;
		...
		if (safeWidth < 12) {
			...
			const roomName = roomNameForId(currentRoomId);
			const comsTag = ` coms [${roomName}] `;
			// left = "┏━" (2) + " coms [name] " (9+name) + leftFill "━" (1) = 12+name
			const tagLen = 12 + roomName.length;
			const left = theme.fg("dim", "┏━") + theme.fg("border", comsTag);
			const leftFill = theme.fg("dim", "━");
			const nameLen = identity ? identity.name.length : 0;
			// rightTag = " " + name + " ━" = 3+name
			const rightTagVisLen = identity ? nameLen + 3 : 0;
			const remaining = safeWidth - tagLen - rightTagVisLen - 1 /* "┓" */;
			if (identity && remaining >= 1) {
				...
				const fallbackRemaining = Math.max(0, safeWidth - 2 /* "┏━" */ - (9 + roomName.length) /* " coms [name] " */ - 1 /* "┓" */);
		...
		if (rows.length === 0) {
			const roomName = roomNameForId(currentRoomId);
			const emptyMsg = theme.fg("muted", `no peers in room "${roomName}"`);
			return [
				topBorder,
	...
	pi.registerTool({
		...
		async execute(_callId, params) {
			...
			const lines = agents.length === 0
				? `No peer agents found in room "${roomNameForId(currentRoomId)}".`
				: agents.map((a) => {
			...
			return {
				...
				details: { agents, project: projectFilter, room: currentRoomId, roomName: roomNameForId(currentRoomId) },
			};
		},
	...
	pi.registerTool({
		...
		async execute(_callId, params) {
			...
			// Room isolation: only allow sends to peers in the same room.
			if (target.room_id && target.room_id !== identity.roomId) {
				throw new Error(`coms: agent "${target.name}" is not in your room ("${roomNameForId(currentRoomId)}")`);
			}
	...
	// ━━ before_agent_start: inject delegation instructions ━━━━━━━━━━━━━━
...
Delegate to peers when ALL of these are true:
- The task naturally splits into **independent subtasks** (separate files, separate research questions, separate code reviews)
- Each subtask requires **non-trivial effort** (not just a single read or 2-line edit)
- Peers have **capacity** — avoid delegating to peers with context_used_pct > 80%

### How to Delegate (Fire-and-Collect Pattern)

1. \`coms_list\` to discover N available peers
2. Split the work into N independent chunks, assign one per peer
3. Fire ALL \`coms_send\` calls first (don't await each one before sending the next)
4. Collect all results with \`coms_await\` on every msg_id
5. Synthesize the results into a final answer

### When NOT to Delegate

- Simple/quick tasks (single file read, small edit)
- Tasks with strong sequential dependencies (step 2 depends on step 1 output)
- Single-file modifications
- No peers available or all peers near context limit
- The overhead of delegation exceeds the work itself

### Coordination Tips

- Give each peer clear, self-contained instructions with specific file paths
- If a peer needs context you have, include it in the prompt
- Prefer \`coms_await\` over polling with \`coms_get\` for simplicity
- If a peer times out or errors, fall back to doing that chunk yourself`;
		...
		handler: async (args, ctx) => {
			...
			if (roomJoinMatch) {
				...
				if (identity) {
					...
					// Update registry entry with new room
					updateOwnRoomId(room.id);
				}
				try { ctx.ui.notify(`coms: joined room "${roomName}"`, "info"); } catch { /* ignore */ }
				// Update status line
				try { ctx.ui.setStatus("coms", `📡 ${identity?.name ?? "?"}@${identity?.project ?? "?"} 🏠 ${roomName}`); } catch { /* ignore */ }
				// Clear stale peer data from the old room before refreshing
				peerCards.clear();
				await refreshPool();
				return;
			}
			...
			// ── /coms room leave ───────────────────────────────────────
			if (trimmed.match(/^room\s+leave/)) {
				const defaultRoom = ensureDefaultRoom(identity?.name ?? "unknown");
				currentRoomId = defaultRoom.id;
				if (identity) {
					identity.roomId = defaultRoom.id;
					updateOwnRoomId(defaultRoom.id);
				}
				try { ctx.ui.notify(`coms: left room — now in "${DEFAULT_ROOM_NAME}"`, "info"); } catch { /* ignore */ }
				try { ctx.ui.setStatus("coms", `📡 ${identity?.name ?? "?"}@${identity?.project ?? "?"} 🏠 ${DEFAULT_ROOM_NAME}`); } catch { /* ignore */ }
				// Clear stale peer data from the old room before refreshing
				peerCards.clear();
				await refreshPool();
				return;
			...
			// ── /coms room rename <new-name> ────────────────────────────
			const roomRenameMatch = trimmed.match(/^room\s+rename\s+(\S+)/);
			if (roomRenameMatch) {
				const newName = roomRenameMatch[1]!;
				const room = readRoom(currentRoomId);
				if (!room) {
					try { ctx.ui.notify(`coms: current room not found`, "error"); } catch { /* ignore */ }
				} else {
					room.name = newName;
					writeRoom(room);
					try { ctx.ui.notify(`coms: room renamed to "${newName}"`, "info"); } catch { /* ignore */ }
					try { ctx.ui.setStatus("coms", `📡 ${identity?.name ?? "?"}@${identity?.project ?? "?"} 🏠 ${newName}`); } catch { /* ignore */ }
				}
				await refreshPool();
				return;
			}
			...
			// ── /coms remove <name> ──────────────────────────────────────
			const removeMatch = trimmed.match(/^remove\s+(\S+)/);
			if (removeMatch) {
				const targetName = removeMatch[1]!;
				const currentProject = displayProject ?? identity?.project ?? "default";
				const projectToRm = currentProject === "*" ? null : currentProject;
				let foundProject: string | null = null;
				let foundEntry: RegistryEntry | null = null;
				if (projectToRm) {
					const entries = readAllRegistryEntries(projectToRm);
					const match = entries.find(e => e.name === targetName);
					if (match) { foundProject = projectToRm; foundEntry = match; }
				} else {
					for (const proj of listProjects()) {
						const entries = readAllRegistryEntries(proj);
						const match = entries.find(e => e.name === targetName);
						if (match) { foundProject = proj; foundEntry = match; break; }
					}
				}
				if (!foundEntry) {
					try { ctx.ui.notify(`coms: agent "${targetName}" not found`, "error"); } catch { /* ignore */ }
				} else {
					removeRegistryEntry(foundProject!, foundEntry.name);
					try { ctx.ui.notify(`coms: removed agent "${targetName}" from project "${foundProject}"`, "info"); } catch { /* ignore */ }
				}
				await refreshPool();
				return;
			}
			...
			if (renameMatch) {
				...
				const currentProject = displayProject ?? identity?.project ?? "default";
				const projectToSearch = currentProject === "*" ? null : currentProject;
				let foundProject: string | null = null;
				let foundEntry: RegistryEntry | null = null;
				if (projectToSearch) {
					const entries = readAllRegistryEntries(projectToSearch);
					const match = entries.find(e => e.name === oldName);
					if (match) { foundProject = projectToSearch; foundEntry = match; }
				} else {
					for (const proj of listProjects()) {
						const entries = readAllRegistryEntries(proj);
						const match = entries.find(e => e.name === oldName);
				...
				if (!foundEntry) {
					try { ctx.ui.notify(`coms: agent "${oldName}" not found`, "error"); } catch { /* ignore */ }
				} else {
					const renamed: RegistryEntry = { ...foundEntry, name: newName };
					writeRegistryAtomic(renamed, foundProject!);
					removeRegistryEntry(foundProject!, oldName);
					try { ctx.ui.notify(`coms: renamed "${oldName}" → "${newName}" in project "${foundProject}"`, "info"); } catch { /* ignore */ }
				}
				await refreshPool();
				return;
			}

			// ── /coms --all (toggle explicit visibility) ──────────────────
			...
			// ── /coms --project <name> (filter project) ───────────────────
